...
class WMT23Dataset(BaseDataset):
    name: str = "wmt23-en-de"
    ...
    def __init__(self, source, target, is_encoder_decoder):
        ...
        self.dataset = Dataset.load_from_disk(f"data/ds/wmt23-en-de")
    ...
    def get_tokenizer(self) -> PreTrainedTokenizerFast:
        ...
        dir = os.path.join("data/mt/tokenizers", f"wmt23-6M-{source}-{target}")
        special_tokens = {
            "pad_token": "<PAD>",
            "eos_token": "<EOS>",
            "sep_token": "<SEP>",
    ...
    def get_dataloaders(
        ...
        assert test == True, "test only dataset"
        assert self.source_lang == "en" and self.target_lang == "de", "only en-de test set"
